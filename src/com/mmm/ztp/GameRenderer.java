 ...
 * 
 */
public class GameRenderer implements Renderer, AddPlayersDrawableEventListener,
		DestroyObjectEventListener, AddAlienDrawableEventListener,
		AddAlienProjectileEventListener {

	float tmp;
	TickerReusable zegarek=new TickerReusable();
	...
	private final LinkedBlockingDeque<BaseObject> alienProjectiles = new LinkedBlockingDeque<BaseObject>(); // pociski
																											// obcych
	...
	public void onDrawFrame(GL10 gl) {
		...
		synchronized (playersObjects) {
			for (Drawable d : playersObjects) {
				d.draw(gl);
			}
		}

		synchronized (alienObjects) {
			for (Drawable d : alienObjects) {
				d.draw(gl);
			}
		}

		synchronized (alienProjectiles) {
	...
	/**
	 * Implementacja metody Renderera, wykonywana przy zmianie powierzchni,
	 * obróceniu ekranu. Tutaj ustawiam widok jako 2d (Orthof)
	 */
	...
	public void onSurfaceCreated(GL10 gl,
		...
		GameEventBus.getInstance().attachToEventBus(
				...
				new AddPlayersDrawableEventHandler(
						(AddPlayersDrawableEventListener) this));

		GameEventBus.getInstance().attachToEventBus(
				DestroyObjectEventListener.class,
				new DestroyObjectHandler(this));

		GameEventBus.getInstance().attachToEventBus(
				AddAlienDrawableEventListener.class,
				new AddAlienDrawableEventHandler(this));

		GameEventBus.getInstance().attachToEventBus(
				AddAlienProjectileEventListener.class,
	...
	private void hittest() {
		...
		if ((alienObjects.size() == 0) && (!reloadEventSent)) {
				...
				.fireEvent(
						...
						new EngineEventObject(
								EngineEventObject.TYPE_NEXT_LEVEL,
								ship));
				new TickerOnetimer(200) {
					public void onDone() {
	 ...
	 * 
	 * @see
	 * com.mmm.ztp.event.destroyobjectevent.DestroyObjectEventListener#destroyObject
	 * (com.mmm.ztp.event.destroyobjectevent.DestroyObjectEventObject)
	 */
	@Override
